...
#### This visualization shows how many bikes are checked out and for how long. The tooltip shows the hours and minutes of trip along with the ride count for each trip duration. The visualization defaults to show trips in length between 0-3 hours, however there is an hour filter that the user can use to look at an length of trip(s).
